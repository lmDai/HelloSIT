        ...
        handler = new Handler(){
            ...
            public void handleMessage(Message msg) {
                if(msg.what == 1){
                    ...
                    adapter = new SecondclassAdapter(class3.this, secondClassList);
                    lv.setAdapter(adapter);
                    lv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                        @Override
                        public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                            SecondClass secondClass = secondClassList.get(position);
                            Intent intent = new Intent(class3.this,SecondclassDisplayActvivity.class);
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.send_request) {
            if(xuehao.length()==10&&mima.length()>=4){
                ...
                Toast.makeText(class3.this,"你还没有输入账号", Toast.LENGTH_SHORT).show();
            }
