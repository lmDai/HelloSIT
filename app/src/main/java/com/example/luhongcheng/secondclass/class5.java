...
public class class5 extends AppCompatActivity implements View.OnClickListener {
        ...
        handler = new Handler(){
            ...
            public void handleMessage(Message msg) {
                if(msg.what == 1){
                    ...
                    adapter = new SecondclassAdapter(class5.this, secondClassList);
                    lv.setAdapter(adapter);
                    lv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                        @Override
                        public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                            SecondClass secondClass = secondClassList.get(position);
                            Intent intent = new Intent(class5.this,SecondclassDisplayActvivity.class);
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.send_request) {
            if(xuehao.length()==10&&mima.length()>=4){
                ...
                Toast.makeText(class5.this,"你还没有输入账号", Toast.LENGTH_SHORT).show();
            }
